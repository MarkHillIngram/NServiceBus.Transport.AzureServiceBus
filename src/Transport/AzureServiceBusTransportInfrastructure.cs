namespace NServiceBus.Transport.AzureServiceBus
{
    ...
    class AzureServiceBusTransportInfrastructure : TransportInfrastructure
    {
        ...
        static readonly Func<string, string> defaultNameSanitizer = name => name;
        ...
        void WriteStartupDiagnostics()
        {
            settings.AddStartupDiagnosticsSection("Azure Service Bus transport", new
            {
                ...
                SubscriptionNameShortener = settings.TryGet(SettingsKeys.SubscriptionNameSanitizer, out Func<string, string> _) ? "configured" : "default",
                RuleNameShortener = settings.TryGet(SettingsKeys.RuleNameSanitizer, out Func<string, string> _) ? "configured" : "default",
        ...
        QueueCreator CreateQueueCreator()
        {
            ...
            if (!settings.TryGet(SettingsKeys.SubscriptionNameSanitizer, out Func<string, string> subscriptionNameSanitizer))
            {
                subscriptionNameSanitizer = defaultNameSanitizer;
            }
            ...
            return new QueueCreator(localAddress, topicName, connectionStringBuilder, tokenProvider, namespacePermissions, maximumSizeInGB * 1024, enablePartitioning, subscriptionNameSanitizer);
        }
        ...
        SubscriptionManager CreateSubscriptionManager()
        {
            ...
            if (!settings.TryGet(SettingsKeys.RuleNameSanitizer, out Func<string, string> ruleNameSanitizer))
            {
                ruleNameSanitizer = defaultNameSanitizer;
            }

            return new SubscriptionManager(settings.LocalAddress(), topicName, connectionStringBuilder, tokenProvider, namespacePermissions, subscriptionNameSanitizer, ruleNameSanitizer);
        }
